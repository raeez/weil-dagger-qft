...
import sympy as s
R=Path(__file__).resolve().parent;W=R.parents[3];S=W/'research-candidates/weil-core020';P=S/'manuscript/weil-forms-and-domains.tex';new=P.read_text();old=(R/'custody019/inputs/04-paper.tex').read_text();sha=lambda p:hashlib.sha256(Path(p).read_bytes()).hexdigest()
...
core=[];count=0
for p in sorted((R/'custody019/baseline-source').rglob('*.tex')):
 q=S/p.relative_to(R/'custody019/baseline-source');a=p.read_text();b=q.read_text();proofs=list(re.finditer(r'\\begin\{proof\}.*?\\end\{proof\}',a,re.S))
...
(R/'adapted-prior-proof-correspondence.json').write_text(json.dumps(dict(native_source_sha256=sha(R/'custody019/inputs/04-paper.tex'),new_source_sha256=sha(P),native_proofs=records,new_proofs=newproofs,core_sources=core,all_core_proofs_exact=count,native_transport=dict(all33_retained=True,byte_identical=32,notation_only_migration=1,all_focused_boundary_proofs_exact=True,legacy_font_occurrences=8,heading_space_commands=new.count('\\Needspace{5\\baselineskip}'))),indent=2)+'\n')
...
(R/'adapted-prior-exact-calculations.json').write_text(json.dumps(rec,indent=2)+'\n')
