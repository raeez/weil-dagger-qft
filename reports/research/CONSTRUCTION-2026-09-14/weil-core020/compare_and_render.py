...
old=fitz.open(R/'custody019/inputs/01-programme.pdf');base=fitz.open(R/'build-baseline/programme.pdf');full=fitz.open(R/'build-full/programme.pdf');stand=fitz.open(R/'build-standalone/weil-standalone.pdf')
...
resumed=[]
for tag,current,priorpath in [('full',full,R/'custody019/build-full/programme.pdf'),('standalone',stand,R/'custody019/build-standalone/weil-standalone.pdf')]:
 prior=fitz.open(priorpath)
 assert len(prior)==len(current)
 comparisons=[]
 for i in range(len(current)):
  a=raster(prior[i]);b=raster(current[i]);assert a==b,(tag,i+1)
  comparisons.append(dict(page=i+1,raster_sha256=b))
 resumed.append(dict(kind=tag,saved_pdf_sha256=sha(priorpath),all_pages_raster_identical=True,pages=comparisons))
(R/'saved-render-comparison.json').write_text(json.dumps(resumed,indent=2)+'\n')
...
(R/'page-comparison.json').write_text(json.dumps(dict(original_pdf_sha256=sha(R/'custody019/inputs/01-programme.pdf'),baseline_pdf_sha256=sha(R/'build-baseline/programme.pdf'),baseline_all320_whole_page_rasters_exact=True,baseline_rasters=baseline,current_pdf_sha256=sha(R/'build-full/programme.pdf'),unchanged_full_pages=same,changed_inherited_pages=changed,unchanged_body_pages=body_same,new_pages=list(range(321,len(full)+1)),rendered_full_pages=required,method='MuPDF1.5 scale; full-page comparison; additional body comparison y82 to height-50'),indent=2)+'\n')
