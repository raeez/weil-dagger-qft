...
\author{Raeez Lorgat}
\date{}
\hypersetup{pdftitle={The Zeta--Weil Form and Nonunital GNS Transfer},
pdfauthor={Raeez Lorgat},
pdfsubject={The zeta explicit formula, positivity, and common domains of nonunital representations}}
\begin{document}
\maketitle
\begin{abstract}
...
Write $\Lambda_{\rm vM}(n)=\log p$ if $n=p^m$ with $p$ prime and $m\geq1$,
and write $\Lambda_{\rm vM}(n)=0$ otherwise.
...
\begin{split}
 ...
 &-\sum_{n\geq2}\frac{\Lambda_{\rm vM}(n)}{\sqrt n}
...
The functions used below are locally given by convergent power series,
or by uniformly convergent limits of such functions.
...
\begin{proof}
For a continuously differentiable function, the real two-variable
integration formula gives
$\int_{\partial U}h(w)\,dw=2i\int_U\partial_{\bar w}h\,dx\,dy$.
The right side vanishes for a holomorphic function.
Apply this identity after removing a small disc around $z$ to
$h(w)/(w-z)$.  On that small circle, write
...
and the displayed estimate.  Removing small discs around all poles gives
the residue statement by their Laurent expansions.
...
\begin{equation}
 ...
 \frac{\zeta'}{\zeta}(s)=-\sum_{n\geq2}\Lambda_{\rm vM}(n)n^{-s}.
 \label{eq:euler}
\end{equation}
...
$\sum\Lambda_{\rm vM}(n)n^{-c}<\infty$ and both test factors are integrable
...
scales $a(\lambda_j+\lambda_k)$.  In matrix norm,
\begin{equation}
...
Let $\mu>0$ be its smallest eigenvalue.  Matrix convergence makes the
left matrix in \eqref{eq:gaussian-gram-limit} differ from $M$ by less
than $\mu/2$ for all sufficiently small $a$.
Hence $G(a)$ is positive definite.
